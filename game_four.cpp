...
#include <QFont>
#include<four.h>
game_four::game_four(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::game_four)
{
    ui->setupUi(this);
...
void game_four::paintEvent(QPaintEvent *ev)
{
...
void game_four::mouseReleaseEvent(QMouseEvent *ev)
{
    if(!tracking)
    {
...
void game_four::availableMove(int x, int y)
{
...
bool game_four::availableMove2(int x, int y)
{
...
void game_four::availableJump(int x, int y)
{
    mem[x][y]=1;
    direction=2;
...
void game_four::availableJump2(int x, int y)
{
    if((!mem[x][y])&&(board[x][y])&&(in(x,y)))
    {
        switch(direction)
...
void game_four::drawPawn(QPainter &painter, int x, int y, QColor color)
{
