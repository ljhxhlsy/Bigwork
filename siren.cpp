...
void siren::paintEvent(QPaintEvent *ev)
{
...
void siren::mouseReleaseEvent(QMouseEvent *ev)
{
    if(!tracking)
    {
...
void siren::availableMove(int x,int y)
{
...
bool siren::availableMove2(int x,int y)
{
...
void siren::availableJump(int x,int y)
{
    mem[x][y]=1;
    direction=2;
...
void siren::availableJump2(int x,int y)
{
    if((!mem[x][y])&&(board[x][y])&&(in(x,y)))
    {
        switch(direction)
...
void siren::drawPawn(QPainter & painter,int x,int y,QColor color)
{
    painter.setBrush(QBrush(color));
    painter.drawEllipse(x-7,y-7,14,14);
}

void siren::on_pushButton_2_clicked()
{
    this->close();
}


void siren::on_pushButton_clicked()
{
    four *next=new four();
    this->close();
    next->show();
