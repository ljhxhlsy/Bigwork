...
#include <QFont>
#include<four.h>
computer_six::computer_six(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::computer_six)
{
    ui->setupUi(this);
    resize(850,600);
...
computer_six::~computer_six()
{
    delete ui;
}

void computer_six::paintEvent(QPaintEvent *ev)
{
...
void computer_six::mouseReleaseEvent(QMouseEvent *ev)
{
    if(!tracking)
    {
...
void computer_six::availableMove(int x, int y)
{
...
bool computer_six::availableMove2(int x, int y)
{
...
void computer_six::availableJump(int x, int y)
{
    mem[x][y]=1;
    direction=2;
...
void computer_six::availableJump2(int x, int y)
{
    if((!mem[x][y])&&(board[x][y])&&(in(x,y)))
    {
        switch(direction)
...
bool computer_six::in(int x, int y)
{
...
void computer_six::drawPawn(QPainter &painter, int x, int y, QColor color)
{
