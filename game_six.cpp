...
#include <QString>
#include <QFont>
game_six::game_six(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::game_six)
{
    ui->setupUi(this);
...
void game_six::paintEvent(QPaintEvent *ev)
{
...
void game_six::mouseReleaseEvent(QMouseEvent *ev)
{
    if(!tracking)
    {
...
void game_six::availableMove(int x, int y)
{
...
bool game_six::availableMove2(int x, int y)
{
...
void game_six::availableJump(int x, int y)
{
    mem[x][y]=1;
    direction=2;
...
void game_six::availableJump2(int x, int y)
{
    if((!mem[x][y])&&(board[x][y])&&(in(x,y)))
    {
        switch(direction)
...
void game_six::drawPawn(QPainter &painter, int x, int y, QColor color)
{
