...
void sanren::paintEvent(QPaintEvent *ev)
{
...
void sanren::mouseReleaseEvent(QMouseEvent *ev)
{
    if(!tracking)
    {
...
void sanren::availableMove(int x,int y)
{
...
bool sanren::availableMove2(int x,int y)
{
...
void sanren::availableJump(int x,int y)
{
    mem[x][y]=1;
    direction=2;
...
void sanren::availableJump2(int x,int y)
{
    if((!mem[x][y])&&(board[x][y])&&(in(x,y)))
    {
        switch(direction)
...
void sanren::drawPawn(QPainter & painter,int x,int y,QColor color)
{
...
void sanren::on_pushButton_2_clicked()
{
    this->close();
}


void sanren::on_pushButton_clicked()
{
    four *next=new four();
    this->close();
    next->show();
